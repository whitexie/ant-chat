...
describe('openAI service', () => {
  ...
  describe('transformMessages', () => {
    it('messages 包含图片', () => {
      const messages = [
        createUserMessage({ convId: '0' as ConversationsId, content: [{ type: 'text', text: 'test1' }] }),
        createUserMessage({ convId: '0' as ConversationsId, content: [{ type: 'text', text: 'test2' }], images: [{ uid: '0', name: 'test', type: 'image/png', size: 123, data: 'https://example.com/image.jpg' }] }),
      ]
    ...
    it('messages 都是text数组', () => {
      const messages = [
        createUserMessage({ convId: '0' as ConversationsId, content: [{ type: 'text', text: 'test1' }] }),
        createUserMessage({ convId: '0' as ConversationsId, content: [{ type: 'text', text: 'test2' }] }),
      ]
  ...
  describe('sendChatCompletions', async () => {
    it('验证request接受到正确的请求参数', async () => {
      ...
      await service.sendChatCompletions([createUserMessage({ convId: '0' as ConversationsId, content: [{ type: 'text', text: 'test1' }] })])
